...
if (isset($_POST['delete'])) {
    $delete = $_POST['delete'];
    $delete_query = "DELETE FROM kassa_1 WHERE id=$delete";
...
if (isset($_POST['barcode'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $barcode = $row['barcode'];
        $prijs = $row['prijs'];
        $check_query = "SELECT * FROM kassa_1 WHERE barcode='$barcode'";
        $check_result = mysqli_query($conn, $check_query);
        if (mysqli_num_rows($check_result) > 0) {
            $kassa_row = mysqli_fetch_assoc($check_result);
            $kassa_aantal = $kassa_row['aantal'];
            $update_query = "UPDATE kassa_1 SET aantal=$kassa_aantal+0 WHERE barcode='$barcode'";
            mysqli_query($conn, $update_query);
        } else {
            $insert_query = "INSERT INTO kassa_1 (id, naam, barcode, aantal, prijs) VALUES ('$id', '$naam', '$barcode', '?', '$prijs')";
        ...
        $update_kassa_1_query = "UPDATE kassa_1 SET aantal=aantal+1 WHERE barcode='$barcode'";
        mysqli_query($conn, $update_kassa_1_query);
    } else {
...
if (isset($_POST['new_customer'])) {
    $copy_query = "INSERT INTO rapport_1 (id, naam, barcode, aantal, prijs) SELECT id, naam, barcode, aantal, prijs FROM kassa_1";
    if (mysqli_query($conn, $copy_query)) {
        $empty_query = "DELETE FROM kassa_1";
...
//maakt de kassa_1 tabel
$table = 'SELECT id, naam, barcode, aantal, prijs FROM kassa_1';
